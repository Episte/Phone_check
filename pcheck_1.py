...
#リース状態&ATA以外の辞書作成
lease_dictionary = {}
for i in range(2,lease_ws.max_row+1):
    if (lease_ws.cell(row=i,column=4).value == "LEASE") \
    and (lease_ws.cell(row=i,column=7).value != "ATA190"):
        lease_dictionary[lease_ws.cell(row=i,column=7).value] \
        = lease_ws.cell(row=i,column=1).value
#リース状態&ATA以外のホスト名を取得
lease_hosts = [i for i in lease_dictionary]
#CUCMの辞書作成
cucm_dictionary = {}
...
for match_phone in match_phones:
    ...
    #パターン2
        ...
        working_phone_ws.cell(row=working_phone_row,column=2).value = lease_dictionary[match_phone]
        #疎通がとれるかチェック
        if checkm_1.Ping(lease_dictionary[match_phone]):
            result = checkm_1.Phone_Check(lease_dictionary[match_phone])
            working_phone_ws.cell(row=working_phone_row,column=3).value = "○"
            for i in range(len(result)):
                working_phone_ws.cell(row=working_phone_row,column=i+4).value\
                = result[i]
...
for lease_only in lease_onlys:
    if cucm_dictionary[lease_only] != "なし" and cucm_dictionary[lease_only] != "不明":
        ...
        no_cucm_ws.cell(row=no_cucm_row,column=2).value = lease_dictionary[lease_only]
        no_cucm_row += 1
#保存して終了
